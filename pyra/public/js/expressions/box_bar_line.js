  ...
  /**
     ...
     * @returns {unresolved}
     */
    add_line_to_box = function(stroke_width, x_buffer, box_width, y_value, svg, scaleY, colour, box_width_wiskers, median, graph,tooltip_box,sample_type_for_hover) {
      // changes done by Isha
        ...
        if (options.bar_graph == "yes") {
          if(median == "no") {
            ...
            .attr("x2", (x_buffer + 15))
                    ...
                    .on('mouseover', function() {
                      tooltip_box[sample_type_for_hover].style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY + 30) + "px").style("display", "inline");
                    })
            ...
            svg.append("line")
            .attr("x1", (x_buffer - box_width/2 - parseInt(stroke_width)/2)) // stroke_width/2 is to cover extra empty corner space on both sides to make the bar graph outline smooth
            .attr("x2", (x_buffer + box_width/2 + parseInt(stroke_width)/2))
                    .attr("y1", scaleY(y_value))
                    .attr("y2", scaleY(y_value))
                    .attr("shape-rendering","crispEdges")
                    .attr("stroke-width",stroke_width)
          ...
          if(median == "no") {
            ...
            svg.append("line")
            .attr("x1", (x_buffer - box_width_wiskers - parseInt(stroke_width)/2))
            .attr("x2", (x_buffer + box_width_wiskers + parseInt(stroke_width)/2))
                    .attr("y1", scaleY(y_value))
                    .attr("y2", scaleY(y_value))
                    .attr("shape-rendering","crispEdges")
                    .attr("stroke-width",stroke_width)
 ...
 */
    add_vertical_line_to_box = function (stroke_width, x_position, y_lower,
            y_upper, svg, scaleY, colour_wiskers, graph,tooltip_box,sample_type_for_hover) {
        svg.append("line")
            .attr("x1", x_position)
            .attr("x2", x_position)
    ...
    add_scatter_line = function (svg, colour, x1, x2, y1, y2, line_stroke_width) {
        svg.append("line")
                ...
                .attr("y1", y1)
                .attr("y2", y2)
                .attr("shape-rendering", "auto") // makes line smooth
  ...
  /**
     ...
     */
    make_scatter_tooltip = function (probe, line_group,day,  sample_type, sample_ids, type) {
        var tooltip_scatter = d3.tip()
                .attr('class', 'd3-tip')
                .offset([0, +110])
                .html(function (d) {
                    probe_name_for_tooltip = options.probe_name_for_tooltip;
                    temp =
                            probe_name_for_tooltip + ": " + probe + "<br/>" +
                            "Line Group: " + line_group + "<br/>" +
                            type + day + "<br/>" +
                            "Replicate ID: " + sample_ids + "<br/>";
                    return temp;
                });
