...
module.exports = biojsvisscatterplot = function (init_options)
{
    ...
    get_x_value_scatter = function (graph, data) {
        ...
        var sample_id = data.Replicate_ID;
    ...
    sort_scatter_data = function (graph) {
        ...
        if (options.sortByOption == null) {
          ...
          sample_type_order = options.sample_type_order.split(',');
          // if sort by is sample_type than order the values based on sample_type_order
          if(sort_by_options.length == 1 && options.sortByOption == "Sample_Type") {
            nested_values = d3.nest()
                    .key(function (d) {
          ...
          // if sort by is other than sample_type than no order is defined
          else if (sort_by_options.length == 1 && options.sortByOption != "Sample_Type") {
            nested_values = d3.nest()
                    .key(function (d) {
                        var value = sort_by_options[0];
                        return d[value];
          ...
          // if multiple sortBy options than sample_type key willbe ordered based on sample_type_order
          else {
            nested_values = d3.nest()
                    .key (function (d) {
                        var value = sort_by_options[0];
    ...
    setup_scatter = function (graph) {
        ...
        svg.selectAll(".dot") // class of .dot
                ...
                .attr("id", function(d) {
                  if (isNaN(d[options.y_column])){return;}
                    return "scatter-point-" + d.Replicate_ID;
    ...
    setup_graph = function (graph) {
        ...
        if (options.display.x_axis_labels === "yes") {
            graph = setup_x_axis_labels(graph, graph.sample_id_list, label_padding, class_name_for_second_sort_by, collective_name, 1);
            if (options.multi_group != 1) {
                label_padding += 80;
        ...
        // Display the legend if the user has specified they want the legend
         if (options.display.legend  === "yes") {
            ...
            var class_name_for_hidden_legend = ".hidden_legend";
            graph = setup_D3_legend(graph, options.legend_list,class_name_for_hidden_legend);
        }
