...
class ParseResult(object):
    ...
    def __init__(self, urls, users, reply, lists, tags, tweet_id, tweet_url, reply_url, retweet_url, favourite_url, relative_time, html):
        ...
        self.favourite_url = favourite_url
...
class Parser(object):
    ...
    def _relative_time(self, created_at):
        tdate = datetime.strptime(created_at,'%a %b %d %H:%M:%S +0000 %Y')
        diff = datetime.utcnow() - tdate
        if diff.days > 100 or diff.days < 0:
            ...
            return 'about {} days ago'.format(diff.days)
        elif diff.seconds <= 1:
            return 'just now'
        elif diff.seconds < 60:
            return 'about {} seconds ago'.format(diff.seconds)
        elif diff.seconds < 120:
            return 'about 1 minute ago'
        elif diff.seconds < 3600:
            return 'about {} minutes ago'.format(diff.seconds/60)
