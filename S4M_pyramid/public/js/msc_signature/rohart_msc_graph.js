...
module.exports = biojsvisrohartmsctest = function(init_options)
{
    ...
    */
    ...
    this.setup_x_axis = function (graph){
        ...
        */
        ...
        svg.selectAll("g") // this is for the PDF
            .attr("class","tick")
            .remove();
    ...
    this.setup_x_axis_using_sample_types = function (graph){
        ...
        svg.selectAll(".sample_type_text")  // text for the xaxes - remember they are on a slant
           ...
           .text(
            ...
            .on('mouseover',function(d) { var tooltip_label = d3.select('body').append("div").attr("id","x-axis-tooltip").attr("class","d3-tip").text(d.sample_type).style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY - 50) + "px").style("display", "inline").style("opacity","1") ;return tooltip_label})
    ...
    this.setup_legend = function(graph){
        ...
        // linear gradient code from here: https://gist.github.com/mbostock/1086421
        var width = 130
            height = 30
            margin_x = -45
            margin_y = 7
            ...
            title_y = -20
            base_x = -110
            base_y = -options.margin.top/2;
            zero_axis_move = 50
            text_width = 100;
            legend_title = "% called MSC";
       ...
       var legend = svg.append("svg:defs")
        ...
        legend.append("svg:stop")
            .attr("offset", "0%")
            .attr("stop-color", options.domain_colours[0])
            .attr("stop-opacity", 1);

        legend.append("svg:stop")
            .attr("offset", "100%")
            .attr("stop-color", options.domain_colours[1])
            .attr("stop-opacity", 1);
        ...
        svg.append("text")
            .attr("x",base_x + title_x)
            .attr("y", base_y+title_y)
            .attr("transform", "translate("+margin_x+","+margin_y+")")
            .attr("width", text_width)
            .attr("text-anchor", "middle")
            .text(legend_title).attr("class","legend_text");
        ...
        svg.append("text")
            ...
            .text(options.legend_range[1]+"%").attr("class","legend_text");

        svg.append("text")
            .attr("x",base_x+width+zero_axis_move)
            .attr("y", base_y)
            .attr("width", text_width)
            .attr("text-anchor", "right")
            .attr("transform", "translate("+margin_x+","+margin_y+")")
            .text(options.legend_range[0]+"%").attr("class","legend_text");
    ...
    */
    ...
    this.setup_svg = function (graph){
        ...
        height_divisor = 1.5;
        svg.append("text")
            .attr("x", options.x_middle_title)
            .attr("y", 0 - (page_options.margin.top /height_divisor)-(height_margin * 2)) 
            .attr("text-anchor", "middle")
    ...
    this.setup_graph = function(graph){
        ...
        // setup_x_axis deleted things in the setup_y_axis.
        // so call setup_y_axis second
