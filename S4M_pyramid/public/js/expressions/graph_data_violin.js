...
function run_violin_gene_expression_graph(colours,graphDiv,sortByOption, show_min_y_axis, ref_name,main_title,scaling_required,dataset_data,probe_name_for_tooltip,ref_type){
  ...
  sample_type_order = dataset_data["sampleTypeDisplayOrder"];
     ...
     //SORTING FOR LINEGRAPH
         data.sort(function(a, b) {
             first = a.Sample_Type;
             second = b.Sample_Type;
             val = first.localeCompare(second);
             return val;
         });
     }
     max = 0;
     ...
     colour_count = 0;
     sample_types = [];
     /* if graph is not multiview, only than sample_type hover object will created,
      otherwise if multiview it uses the sample_type_hover object defined in graph_multi_view.js
       so that sample types for all the datasets can be merged */
     ...
     data.forEach(function(d){
       ...
       if ($.inArray(d.Sample_Type, sample_types) == -1) {
            sample_type_count ++;
            sample_types.push(d.Sample_Type);
    ...
    var sort_colour = {};
    sort_colour = setup_colours_for_group(sort_option_array.sort(), sort_colour,
    ...
    //     number_of_colours);
    var sample_colour = {};
    sample_colour = colours;
    var line_group_colour = {};
    // line_group_colour = setup_colours_for_group(line_group_array.sort(), line_group_colour,
    //     number_of_colours);


   //Need a name of all sample_type states for the sample type
   for (sample_type in sample_types) {
       sample_type_names = sample_types[sample_type] + " " + sample_type_names;
   }
   ...
   legend_list: {name: first_option, list: sort_option_array},//{name: "sample_types", list: Object.keys(dataset_data["sampleTypeDisplayGroups"]).sort(function(a,b){return dataset_data["sampleTypeDisplayGroups"][a]-dataset_data["sampleTypeDisplayGroups"][b]})},
   colour_array: sort_colour,//sample_colour,
   tip_decoy: tip,
   legend_text: "yes",
   ...
   radius: 4,
   ref_name: ref_name,
       level_of_overlap: 0.1,
   sort_by_sample_id: "no",
   y_axis_largest_value: y_axis_largest_value,
   ...
   height: 400,
   // Old legend list
  //  legend_list: sample_types,//Select the list you want on the legend (i.e. sample_id_list)
   x_axis_list: unique_probes_sorted,
   legend_probe_tip: "none",
       width: graph_box_width,
       margin:{top: 250, left: 130, bottom: 300, right: 270},
       initial_padding: 10,
       ...
       display: {hoverbars: "yes", error_bars: "no", legend: "yes", horizontal_lines: "yes", vertical_lines: "yes", x_axis_labels: "yes", y_axis_title: "yes", horizontal_grid_lines: "yes"},
       ...
       box_width: 50,
       box_width_wiskers: 5,
       sample_type_order: dataset_data["sampleTypeDisplayOrder"], //Order of the sample_type state on the x axis
       ...
       //Including the sample_type state on the x axis causes the order to change as the data becomes
       //sorted by probes and sample_type state
       include_sample_type_x_axis: "no", //Includes the sample_type state on the x axis
       size_of_sample_type_labels: 200, //The size allotted to the sample_type state labels
       x_axis_padding: 50,
...
*/
   ...
   var instance = biojsvisviolingraph(options);
