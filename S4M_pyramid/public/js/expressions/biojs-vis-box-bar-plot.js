...
module.exports = biojsvisboxplot = function (init_options)
{
 ...
 /*------------------------------Box plot Calculations--------------------------------------*/
    ...
    add_scatter_to_box = function(svg, graph, scatter_values, median_line, sample_type, colour, colour_stroke) {
        ...
        svg.selectAll(".dot") // class of .dot
           ...
           .on('mouseover', function() {
             tooltip_box[sample_type].style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY + 30) + "px").style("display", "inline");
           })
           .on('mouseout', function() {
             tooltip_box[sample_type].style("display","none");
           })
    ...
    make_box_tooltip = function(probe, sample_type, second_sort_by,nan_counter) {
        if(options.sortByOption.split(",").length == 1) {
            var state = options.sortByOption;
        }
        else {
            state = options.sortByOption.split(",")[1];
        }
        var tooltip_box = d3.select('body').append("div").attr("id",probe+"_"+sample_type).attr("class","d3-tip").style("display","none")
            .html(function(d) {
              var probe_name_for_tooltip = options.probe_name_for_tooltip;
              temp = probe_name_for_tooltip+ ": " + probe + "<br/>" ;
               if(nan_counter == 0){ // when no none expression values are there
                 if((sortBy == "Sample_Type") || (sortBy == "Sample Type") ||(options.sortByOption.split(",").length != 1)) { // shows sample type long if grouped by sample type
                    temp = temp + "Sample Type" + " : " + sample_type + " ["+ sample_type_hover[sample_type] + "] " +"<br/>"
                 }
                 else {
                   temp = temp + state + ": " + sample_type +"<br/>"
                 }
                  ...
                  if((sortBy == "Sample_Type") || (sortBy == "Sample Type") ||(options.sortByOption.split(",").length != 1)) { // shows sample type long if grouped by sample type
                     temp = temp + "Sample Type" + " : " + sample_type + " ["+ sample_type_hover[sample_type] +"] " + "<br/>" +nan_counter +" Sample Removed Due to Floored Values " + "<br/>"
                  }
                  else {
                    temp = temp + state + ": " + sample_type +"<br/>" +nan_counter +" Sample Removed Due to Floored Values " + "<br/>"
                  }
        ...
        return tooltip_box; // this is converted to div so that mousemove can move the div anywhere along with cursor
    }


    draw_box = function (svg, graph, box_plot_vals, x_buffer, sample_type_for_hover,tooltip_box) {
        ...
        svg.append("rect")
            ...
            .attr("fill", function() {
              if((options.sortByOption.split(',')[0] == 'Sample_Type' || options.sortByOption.split(',')[1] == 'Sample_Type'))
                {return options.colour[sample_type_for_hover];}
              else
                {return options.colour_array[sample_type_for_hover]}
            })
            .attr("opacity", opacity)
            .on("mouseover", function(d) {
              tooltip_box[sample_type_for_hover].style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY + 30) + "px").style("display", "inline"); // tooltip is converted to tooltip div  that can be moved anywhere if required such as following cursor
            })
            .on("mouseout", function(d) {
              tooltip_box[sample_type_for_hover].style("display","none")
            })
            .on("mousemove",function(d) {
              tooltip_box[sample_type_for_hover].style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY + 30) + "px").style("display", "inline"); // added mousemove to be followed by tooltip div
            });
	return svg;
    }

    /* Draw box plot draws the box and wiskers onto the graph and also if it is a bar graph this is drawn on too */
    draw_box_plot = function(samples, graph, box_plot_vals, probe_name, sample_type_for_hover, second_sort_by_name, nan_counter) {
        var svg = graph.svg;
        scaleY = graph.scaleY;
        scaleX = graph.scaleX;
        options = graph.options;

        tooltip_box = {}; // T#2701 - have to reset tooltip_box for every probe. Otherwise the last probe shows up in all the hover overs.
        tooltip_box[sample_type_for_hover] = make_box_tooltip(probe_name, sample_type_for_hover, second_sort_by_name,nan_counter);
        ...
        if(options.sortByOption.split(",").length != 1) {
          for(i=0; i<nested_values[0].values.length; i++) {
            ...
            var id = remove_chars(probe_name + "-" + sample_type_for_hover + "-" + second_sort_by_name);
            var name = map[id];
            x_buffer = box_scale(name) - box_width/2;
        } else {
            var id = remove_chars(probe_name + "-" + sample_type_for_hover);
            var name = map[id];
            x_buffer = box_scale(name) - box_width/2;
        }
        ...
        // changes done by Isha
        // colour_box = sample_types_with_colour[sample_type_for_hover];
        // colour_box = options.colour_array[sample_type_for_hover];
        if(options.whiskers_needed == true) {
          if((options.sortByOption.split(',')[0] == 'Sample_Type' || options.sortByOption.split(',')[1] == 'Sample_Type'))
            {colour_wiskers = options.colour[sample_type_for_hover];
            colour_box = options.colour[sample_type_for_hover]} // this is when sample type is legend and colour is gradient
          else
            {colour_wiskers = options.colour_array[sample_type_for_hover];
            colour_box = options.colour[sample_type_for_hover]} // this when tissue, gender etc is present and we do not need gradient
        }
        else {
          colour_wiskers = undefined;
          colour_box = options.colour[sample_type_for_hover]
        }
        colour_median = "white";
        id = probe_name + "-" + sample_type_for_hover + "-" + second_sort_by_name;
        //Add vertical lline
        if (options.probe_count == 1 && options.sortByOption.split(",").length == 1 && options.bar_graph == "yes") {
            ...
            box_group = add_vertical_line_to_box(options.stroke_width, x_buffer + box_width*0.5,
                    box_plot_vals[0], box_plot_vals[2], box_group, scaleY,colour_wiskers, graph,tooltip_box,sample_type_for_hover);
        }
        else if (options.bar_graph == "yes") {
            opacity = 0.4;
            box_group = add_vertical_line_to_box(options.stroke_width, x_buffer+ box_width*0.5,
                    box_plot_vals[0], box_plot_vals[2], box_group, scaleY,
                colour_wiskers, graph,tooltip_box,sample_type_for_hover);
        } else {
            opacity = 1;
        ...
        //Add box
        box_group = draw_box(box_group, graph, box_plot_vals, x_buffer, sample_type_for_hover,tooltip_box);
        //Add min line
        if (options.bar_graph == "yes") {
              if(box_width < 8){
                if (box_plot_vals[0] != box_plot_vals[2]){
                  //Add min line
                  box_group = add_line_to_box(options.stroke_width, x_buffer, box_width, box_plot_vals[0], box_group, scaleY, colour_wiskers, box_width_wiskers,"no", graph,tooltip_box,sample_type_for_hover);
                  //Add max line
                  box_group = add_line_to_box(options.stroke_width, x_buffer,box_width,
                      ...
                      "no",graph,tooltip_box,sample_type_for_hover);
                }
              }
              else {
                //Add min line
                box_group = add_line_to_box(options.stroke_width, x_buffer
                    ...
                    "yes", graph,tooltip_box,sample_type_for_hover);
                //Add max line
                box_group = add_line_to_box(options.stroke_width, x_buffer
              ...
              //Add median lines
              box_group = add_line_to_box(options.stroke_width, x_buffer, box_width,
                    box_plot_vals[1], box_group, scaleY, colour_box, box_width_wiskers,"yes", graph,tooltip_box,sample_type_for_hover);

            //Add outside lines
            box_group = add_vertical_line_to_box(options.stroke_width, x_buffer, 0,
                box_plot_vals[1], box_group, scaleY, colour_box, graph,tooltip_box,sample_type_for_hover);
            box_group = add_vertical_line_to_box(options.stroke_width, x_buffer
                + box_width, 0, box_plot_vals[1], box_group, scaleY, colour_box, graph,tooltip_box,sample_type_for_hover,sample_type_for_hover);
        } else {
          if(box_width < 8){
            //Add max line
             box_group = add_line_to_box(options.stroke_width, x_buffer, box_width,
                 box_plot_vals[0], box_group, scaleY, colour_wiskers, box_width_wiskers,"no", graph,tooltip_box,sample_type_for_hover);
             //Add max line
             box_group = add_line_to_box(options.stroke_width, x_buffer, box_width,
                 box_plot_vals[4], box_group, scaleY, colour_wiskers, box_width_wiskers,"no", graph,tooltip_box,sample_type_for_hover);
          }
          else {
            //Add max line
             box_group = add_line_to_box(options.stroke_width, x_buffer, box_width,
                 box_plot_vals[0], box_group, scaleY, colour_wiskers, box_width_wiskers,"yes", graph,tooltip_box,sample_type_for_hover);
             //Add median line
             box_group = add_line_to_box(options.stroke_width, x_buffer
                 , box_width, box_plot_vals[2], box_group, scaleY, colour_median,
                 (box_width/2.0)*median_percent,"yes", graph,tooltip_box,sample_type_for_hover);
             //Add max line
             box_group = add_line_to_box(options.stroke_width, x_buffer, box_width,
                 box_plot_vals[4], box_group, scaleY, colour_wiskers, box_width_wiskers,"yes", graph,tooltip_box,sample_type_for_hover);
          }
        }
        //Option to allow the user to test their values
        if (options.test == "yes") {
            test_values(second_sort_by_name + " " + probe_name + "|" + sample_type_for_hover, box_plot_vals, graph, options);
        }
 ...
 /** --------------------------- ADDED to make modular ----------------------------- */
    ...
    setup_graph = function(graph){
        ...
        if(multiview_graph == "no") {
          tooltip_box = {}; //only create for non multi view graph, otherwise it uses tooltip_box defined in multiview js file
        }
        var x_axis_position = "bottom";
        var class_name = ".probe_text";
        var class_name_for_second_sort_by = ".second_sort";
        ...
        graph = setup_x_axis_labels(graph, null, label_padding, class_name_for_legend, collective_name_for_legend, 3, x_axis_position);
     	...
     	if (options.sortByOption.split(",").length != 1) {
                    label_padding += 120;
		    graph = setup_x_axis_labels(graph, null, label_padding, class_name_for_second_sort_by, collective_name, 2, x_axis_position);
            // Delete extra padding to the label
                    label_padding -= 120;
        }
        label_padding = label_padding - 100; // this is to adjust padding for top x axis
        graph = setup_x_axis_labels(graph, null, label_padding, ".hidden-probe_text", collective_name, 1, x_axis_position);
