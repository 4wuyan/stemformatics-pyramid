    ...
    /**
     * removes all special characters except space)
     */
    remove_chars_except_space = function (name) {
        newstring = name.replace(/[&\/\\#,()$~%.'":*?<>{}+]/g,'');///[^\w\-\+]/g, '')///[&\/\\#,+()$~%.'":*?<>{}\s+]/g,'');
        return newstring;
    }
    ...
    */
    setup_data_for_x_axis = function(graph){
        ...
        var multi_xaxis_scale_with_space = [];
        ...
        var sample_type;
        var vertical_lines = [];
        inner_label = []; // these are the second sort by x axis label
        outer_label = []; // these are the main x axis label (probe for box/bar, sample type for scatter)
        very_inner_label = []; // these are the legend on x axis label
        var start_name;
        var outer_start_name;
        var cur_name;
        var tempX_outer;
        if(multiview_graph == "no") {
          probeInfo= {};
        }
        else {
          if(typeof probeInfo == "undefined"){ // only create this variable if it is not already created for multiview graphs
            probeInfo = {};
          }
        }
            ...
            if (options.multi_group != 1) { //There will be a second key which is the second_sort_by state
                ...
                for (var st in values) {
                    ...
                    for (var ds in dvals) {
                        ...
                        cur_name_with_space = remove_chars_except_space(probe + "-" + sample_type + "-" + second_sort_by);
                        ...
                        // adding hover overs for box/bar/violin
                        if (graph.graph_type != "Scatter Plot") {
                            inner_tempX['hover'] = sample_type_hover[sample_type];
                            tempX['hover'] = second_sort_by;
                        }
                        else {
                            tempX['hover'] = sample_type_hover[second_sort_by];
                        }
                        if (graph.graph_type != "Scatter Plot") {
                            tempX['multi_mapping'] = nested_values[p].values[0].values[0].values[0]['Multi_Mapping']; // get the mapping value pf probe p from the first sample, as the mapping value of probe is always unique
                            tempX['Mapped_gene'] = nested_values[p].values[0].values[0].values[0]['Mapped_gene'];
                        }
                        if(graph.graph_type == "Scatter Plot") {
                          if(multiview_graph == "no") { // when it is not multiview_graph we will create probeInfo object for the probes in the legend list only
                                  ...
                                  // storing multi map information so that can be used in axis label and legend to show * and red colour
                                      for(var i=0; i< nested_values[p].values[0].values[0].values.length; i++) {
                                           if(legend_list[legend] == nested_values[p].values[0].values[0].values[i]["Probe"]) {
                                             ...
                                             probeInfo[legend_list[legend]]['multi_mapping'] = nested_values[p].values[0].values[0].values[i]['Multi_Mapping'];
                                             probeInfo[legend_list[legend]]['Mapped_gene'] = nested_values[p].values[0].values[0].values[i]['Mapped_gene'];
                                          }
                                      }
                                }
                              }
                          }
                          else { // when it is multiview_graph we will create probeInfo object for the probes in the all the graphs. So we keep appending to same probeInfo object again and again
                            var probes =nested_values[p].values[0].values[0].values;
                            for(var i=0; i< probes.length; i++) {
                              if(!probeInfo[probes[i]["Probe"]]){
                                  probeInfo[probes[i]["Probe"]] = [];
                                  probeInfo[probes[i]["Probe"]]['label'] = probes[i]["Probe"];
                                  probeInfo[probes[i]["Probe"]]['multi_mapping'] = probes[i]['Multi_Mapping'];
                                  probeInfo[probes[i]["Probe"]]['Mapped_gene'] = probes[i]['Mapped_gene'];
                              }
                            }
                          }
                        ...
                        if ($.inArray(cur_name_with_space, multi_xaxis_scale) == -1) {
                            multi_xaxis_scale_with_space.push(cur_name_with_space);
                        }
                ...
                if (graph.graph_type != "Scatter Plot") {
                    tempX['multi_mapping'] = nested_values[p].values[0].values[0].values[0]['Multi_Mapping']; //find the Multi_Mapping value for probe p
                    tempX['Mapped_gene'] = nested_values[p].values[0].values[0].values[0]['Mapped_gene']; // find the mapped genes
                    tempX_outer['multi_mapping'] = nested_values[p].values[0].values[0].values[0]['Multi_Mapping']; // get the mapping value pf probe p from the first array, as the mapping value of probe is always unique
                tempX_outer['Mapped_gene'] = nested_values[p].values[0].values[0].values[0]['Mapped_gene']; // get the multi mapping gene names for probe p
                }

                tempX_outer['hover'] = probe;
                ...
                for (var st in values) {
                    ...
                    cur_name_with_space = remove_chars_except_space(probe + "-" + sample_type);
                    tempX = {}
                    inner_tempX = {}
                    tempX['label1'] = probe;
                    inner_tempX['label1'] = sample_type;
                    ...
                    if (graph.graph_type != "Scatter Plot") {
                        if (graph.graph_type == "Line Graph"){
                            tempX['multi_mapping'] = nested_values[p].values[0].values[0].values[0]['Multi_Mapping']; // get the mapping value pf probe p from the first array, as the mapping value of probe is always unique
                            tempX['Mapped_gene'] = nested_values[p].values[0].values[0].values[0]['Mapped_gene']; // get the multi mapping gene names for probe p

                        } else {
                            tempX['multi_mapping'] = nested_values[p].values[0].values[0]['Multi_Mapping']; // get the mapping value pf probe p from the first array, as the mapping value of probe is always unique
                            tempX['Mapped_gene'] = nested_values[p].values[0].values[0]['Mapped_gene']; // get the multi mapping gene names for probe p
                        }
                        ...
                        else { // when second sort by is the only sort by option
                            inner_tempX['hover'] = sample_type;
                        }
                    }
                    else {
                        if(sortBy  == "Sample_Type") {
                            tempX['hover'] = sample_type_hover[probe];
                        }
                        else {
                            tempX['hover'] = probe;
                        }
                    ...
                    if(graph.graph_type == "Scatter Plot") {
                      if(multiview_graph == "no") {
                            ...
                            // storing multi map information so that can be used in axis label and legend to show * and red colour
                                for(var i=0; i< nested_values[p].values[0].values.length; i++) {
                                    if(legend_list[legend] == nested_values[p].values[0].values[i]["Probe"]) {
                                       probeInfo[legend_list[legend]] = [];
                                       probeInfo[legend_list[legend]]['label'] = legend_list[legend];
                                       probeInfo[legend_list[legend]]['multi_mapping'] = nested_values[p].values[0].values[i]['Multi_Mapping'];
                                       probeInfo[legend_list[legend]]['Mapped_gene'] = nested_values[p].values[0].values[i]['Mapped_gene'];
                                    }
                                }
                          }
                        }
                        ...
                        var probes =nested_values[p].values[0].values;
                        for(var i=0; i< probes.length; i++) {
                          if(!probeInfo[probes[i]["Probe"]]){
                              probeInfo[probes[i]["Probe"]] = [];
                              probeInfo[probes[i]["Probe"]]['label'] = probes[i]["Probe"];
        ...
        var name_mapping_with_space = create_name_mapping(multi_xaxis_scale_with_space);
        graph.vertical_lines_mapping = vertical_lines_mapping;
        graph.multi_xaxis_list = multi_xaxis_scale_with_space;
        graph.vertical_lines = vertical_lines;
        graph.name_mapping = name_mapping.map;
        graph.name_mapping_with_space = name_mapping_with_space.map;
  ...
  /**
    ...
    return_y_min_max_values = function (graph) {
        ...
        lwr_min_max_values_from_data = d3.extent(options.data,
                function (d) {   // this will go through each row of the options.data
                    ...
                    if(isNaN(temp) && isNaN(d.Expression_Value)) { // handle graph lower y axis when all values are NaN
                      temp = 0;
                    }
                    return temp;
                }
        ...
        upr_min_max_values_from_data = d3.extent(options.data,
                function (d) {
                    ...
                    temp = upr;
                    if(isNaN(d.Expression_Value) && isNaN(temp)) { // when all values are NaN in graph
                      temp = 0;
                    }
                    if (upr > max_val) {
                        max_val = upr;
                    }
                    if(isNaN(temp) && isNaN(d.Expression_Value)) { // handle graph upper y axis when all values are NaN
                      temp = 0;
                    }
    ...
    setup_y_axis = function (graph) {
        ...
        if (options.display.y_axis_title === "yes") {
            svg.append("text")
                    ...
                    // .attr("stroke", "black") - commenting as it does not shows properly when downloaded
        ...
        if (options.display.horizontal_grid_lines === "yes") {
            var yAxisNodes = svg.append("g")
                    ...
                    .attr("stroke-width", "0.5") // with prince 1 px stroke become blurry thus reduced size
                    .call(yAxis); //implementing the y axis as an axis
            yAxisNodes.selectAll('text').style({'stroke-width':'0','font-family':'Arial','font-size':'12px'})
        } else {
            svg.append("g")
                .call(yAxis); //implementing the y axis as an axis
        }
        // move y axis by certain padding distance
        var y_axis_padding = -5;
        $(".tick text").attr("transform","translate("+y_axis_padding+",0)")
        graph.svg = svg;
        graph.scaleY = scaleY;
        graph.yAxis = yAxis;
        return graph;
    ...
    return_label_name = function(d,i,label_num,graph,option) {
      delimiter = " ";// this splits the word on space
      if(label_num == 1) {counter_probe ++; }
      ...
      if (label_num == 1 || label_num == 3) {
          ...
          if( option == 1) // returns gene and probe name STAT1 ILMN_1543256
            {
          ...
          else if(option == 2 ) { // return ILMN_565872
                ...
                if (d.multi_mapping == "yes") {
                  if(ref_type == "gene_set_id") {
                    return "Multimapped " + probe_name + "*";
                  }
                  else {
                    return temp + counter_probe + "*";
                  }
                } else {
                    return temp + " " + counter_probe;
                }
                ...
                if(graph.graph_type == "Scatter Plot" || label_num == 3 || label_num == 2) {
                  ...
                  word = prefix;
                  temp = word.replace(regex_function(legend_name_length,delimiter), "$&\n");
                if (d.multi_mapping == "yes" && label_num == 1) {
                    return "Multimapped " + temp + " " + counter_probe + "*";
                } else {
                    if(label_num == 1) {
                        return temp + " " + counter_probe; }
                    else {
    ...
    setup_x_axis_labels = function (graph, label_list, height_offset, class_name, collective_name, label_num) {
        ...
        if(class_name == '.hidden-probe_text') {
          var probe_opacity = 0;
        }
        else {
          var probe_opacity = 1;
        ...
        var angle = -45;
        var x_axis_label_option = 4;
        if(ref_type == "probeID") {
            x_axis_label_option = 3; // return STAT1 peak
        }
        else {
            x_axis_label_option = 4;
        }
        ...
        svg.selectAll(class_name)  // text for the xaxes - remember they are on a slant
                ...
                .attr("class", "x_axis_diagonal_labels x_axis_label " + class_name.replace('.',''))
                .style("text-anchor", "end")
                .style("fill", function(d){
                  if(d.multi_mapping == "yes" && label_num == 1) {
                    ...
                    return '#000000'; // if a colour is not returned for attribute fill, Safari 9.1 does not uses default black colour, and instead breaks as no colour is defined and not showing the label
                  }
                })
                .attr("id", function(d) {
                ...
                .attr("y", function() {
                  if(position == "top" && label_num == 1) {
                    if(angle == -45) { y_value =  0 - height_offset -100}
                    else { y_value =  0 - height_offset;}
                  }
                  else {
                     y_value =  page_options.height + height_offset;
                  }
                    return y_value;
                })
                .attr("x",
                        function (d, i) {
                            ...
                            x_value = multi_scaleX(start) + diff;
                            return x_value ;
                        }
                ) // when rotating the text and the size
                .text(
                  function(d,i){
                    if(ref_type == "probeID" && d.multi_mapping == "yes" && class_name != '.hidden-probe_text' ) {
                        x_axis_label_option = 4;
                        var text =  return_label_name(d,i,label_num,graph,x_axis_label_option) //this will show word multimapped insted of all gene names
                    }
                    else if (class_name == '.hidden-probe_text' ){
                       if (ref_type == "gene_set_id" || ref_type == "probeID") {
                            x_axis_label_option = 1;
                            var text =  return_label_name(d,i,label_num,graph,x_axis_label_option);
                        }
                        else {
                              x_axis_label_option = 2;
                              var text =  return_label_name(d,i,label_num,graph,x_axis_label_option);
                        }
                    }
                    else {
                        if (ref_type == "gene_set_id") {
                            x_axis_label_option = 3;
                             var text =  return_label_name(d,i,label_num,graph,x_axis_label_option);
                        }
                        else {
                            var text =  return_label_name(d,i,label_num,graph,x_axis_label_option);
                ...
                .on("click",function(d){
                  ...
                  if ($("#store_and_submit_select_probes").is(":visible") && class_name== ".probe_text"){
                    var text = "";
                    ...
                    if(d.multi_mapping == "yes") { // mapping will be yes only for probes, thus if sample type is on x axis, it won't have this click function
                      ...
                      new_url = url.split(BASE_PATH)[0]+ "//" + url.split(BASE_PATH)[2] + BASE_PATH +  "probes/multi_map_summary?axis=934&probe_id="+temp_label+"&ds_id="+ds_id+"&db_id="+db_id;
                ...
                .attr("transform",
                        ...
                        function (d, i)  {
                            ...
                            if(position == "top" && label_num == 1) {
                              ...
                              if (options.probe_count == 1 && class_name != '.hidden-probe_text' ){x_axis_text_angle = 0;} // this when single probe it will be sit 0 degrees on top axis
                            }
                            else {
                ...
                .style("text-anchor",function() {
                                        if(label_num == 1 && (graph.graph_type != "Scatter Plot")) {return "start";}
                                        else { return "end";}
                                                })
                ...
                /* Sets up the tooltips to display on the mouseover of the sample type label. This tooltip
                 ...
                 is the sample type */
                //for rotated axis label mouse location is used as position for hover
                .on('mouseover',function(d) {
                                var hover_text_for_multi_mapped = " Click to view multimapped genes";
                                tooltip_label_for_axis = d3.select('body').append("div").attr("id","x-axis-tooltip").attr("class","d3-tip").html(function(){
                                  if(ref_type == "gene_set_id") {
                                    if(d.multi_mapping == "yes") {
                                      return  d.hover+ " <br/>" + hover_text_for_multi_mapped;
                                    }
                                    else
                                    {
                                      return d.hover ;
                                  ...
                                  }}).style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY + 30) + "px").style("display", "inline") ;
                                return tooltip_label_for_axis})
         	.on('mouseout', function(d) { $('#x-axis-tooltip').remove()})
          .on('mousemove',function(d){
            tooltip_label_for_axis.style("left", (d3.event.pageX -50) + "px").style("top", (d3.event.pageY + 30) + "px").style("display", "inline") ;
                          });
