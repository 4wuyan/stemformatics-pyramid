...
<%def name="includes()">   
    ...
    <div id="msc_values_access" class="hidden">${c.msc_values_access}</div> 
    ...
    <div id="wb_background" class="wb_background_divs">    
        <div id="wb_background_inner_div">
            ...
            <div class="export_more_info"><a href="${h.url('/admin/update_datasets')}">Show All</a></div> 
...
mgeg_url = h.url('/workbench/histogram_wizard?graphType=default&db_id='+str(db_id)+'&gene_set_id='+str(gene_set_id)+'&datasetID='+str(ds_id)) 
                        ...
                        <tr>    
                            <td>${ds_id}</td> 
                            <td id="${ds_id}title">${c.datasets[ds_id]['title']}</td> 
                            <td id="${ds_id}handle">${c.datasets[ds_id]['handle']}</td> 
                            <td id="${ds_id}private">${'Private' if c.datasets[ds_id]['private'] else 'Public'}</td> 
                            <td id="${ds_id}published">${'Published' if c.datasets[ds_id]['published'] else 'Not Published'}</td> 
                            <td id="${ds_id}yugene">${'Show Yugene' if c.datasets[ds_id]['show_yugene'] else 'Hide Yugene'}</td> 
                            <td id="${ds_id}limited">${'Show Limited' if c.datasets[ds_id]['show_limited'] else 'Hide Limited'}</td> 
                            <td id="${ds_id}genepattern_access">${c.datasets[ds_id]['gene_pattern_analysis_access']}</td> 
                            <td id="${ds_id}msc_access">${c.datasets[ds_id][c.msc_values_access]}</td> 
                            <td id="${ds_id}platform_type">${c.datasets[ds_id]['platform']}</td> 
                            <td>
...
%if not c.datasets[ds_id]['private']:
...
%endif
                                            ...
                                            <li><a href="#" class="toggleMSC" ds_id="${ds_id}">Toggle MSC</a></li> 
